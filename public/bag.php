...
$db = mysqli_select_db($conn,"checkout");
$sql = 'SELECT * FROM checkout';
$retVal = mysqli_query($conn,$sql) or die( mysqli_error($conn));


while($row = mysqli_fetch_array($retVal, MYSQLI_BOTH)){
    array_push($prodquantiy, $row[1]);
    array_push($prodprice, $row[2]);

}

mysqli_free_result($retVal);
$value = 0;
for($x = 0 ; $x<=$$prodquantiy;$x++){

	if(isset($prodquantiy[$x])){
		$value =$value + $prodquantiy[$x]*$prodprice[$x];
		echo $value;
}
    ...
    <div class="row no-gutters">
        <div class="col-md-8">
            <div class="product-details mr-2">
                ...
                <div class="d-flex justify-content-between"><span>You have X</span>
        ...
        <div class="col-md-4">
            <div class="payment-info">
                ...
                <div class="d-flex justify-content-between information"><span>Shipping</span><span></span></div>
                <div class="d-flex justify-content-between information"><span>Total(Incl. taxes)</span><span></span></div><button class="btn btn-primary btn-block d-flex justify-content-between mt-3" type="button"><span>$3020.00</span><span>Checkout<i class="fa fa-long-arrow-right ml-1"></i></span></button>
            </div>
        </div>
    </div>
</div>
